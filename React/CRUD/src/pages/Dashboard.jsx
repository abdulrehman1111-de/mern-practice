...
const handleSubmtiOnEdit = (e, obj)=>{
    e.preventDefault();
    setUsers(users.map((u)=> u.id === obj.id ? obj:u));
}
...
const handleEdit = (item)=>{
    ...
    setAddress(item.address.street);
                    ...
                    <tbody>
                        {
                            users.map((item, index) => {
                                return <tr className='border-1 border-white' key={index}>
                                    ...
                                    }}><Pencil size={16} className='text-white' /></td>
                                    <td></td>
                                </tr>
                            })
                        }
        ...
        <div className={`fixed top-0 overflow-hidden bg-linear-to-r from-[#0B1120] to-[#0D1526] h-screen w-90 z-11 top-0 transition-all duration-500 ease-out text-white p-2 ${open ? "left-[72%]" : "left-full"}`}>
            ...
            <div className='flex flex-col gap-5 m-3'>
                ...
                <form action="" onSubmit={handleSubmit}>
                    <div className='flex flex-col gap-5'><input className='border-1 border-gray-700 p-3 w-full rounded-md' type="text" name="name" value={name} id='name' placeholder='Full name' onChange={(e) => setName(e.target.value)} />

                        <input className='border-1 border-gray-700 p-3 w-full rounded-md' value={email} type="email" name="" id="email" placeholder='Email' onChange={(e) => setEmail(e.target.value)} />

                        <input className='border-1 border-gray-700 p-3 w-full rounded-md' value={city} type="text" name="city" id='city' placeholder='City' onChange={(e) => setCity(e.target.value)} />

                        <input className='border-1 border-gray-700 p-3 w-full rounded-md' value={address} type="text" name="address" id='address' placeholder='Address' onChange={(e) => setAddress(e.target.value)} /></div>
                </form>
            ...
            <div onClick={(e) => {
                 ...
                 }} id='addBtn' className='w-[90%] relative top-50 ml-4 h-10 rounded-4xl bg-linear-to-r from-[#111C2E] to-[#131F35] hover:bg-none hover:bg-[#364e7c] flex justify-center items-center text-center font-semibold'>
                <p className='text-sm'>
