...
const TimezoneDiv = ({ getter, setter }) => {
                        ...
                        : selectedValue.map((item, index) => {
                            return <>
                                <div className='flex justify-between'>
                                    ...
                                    <input value={item}
                                        type="radio" checked={item === getter} onChange={() => setter(item)} name="" id={`timezone-${index}`} className='w-5 h-5 appearance-none rounded-full border-1 border-white checked:bg-green-400' />
                                </div>
                            </>
                        })
                }
